...
class TestDeltaLeakyCore:
    """Core tests for DeltaLeaky neuron"""
    ...
    def test_spike_condition(self, step_input):
        ...
        spikes = [delta(step_input[i]).item() for i in range(len(step_input))]
        assert sum(spikes) > 0, "Never spiked"
    ...
    def test_threshold_sensitivity(self, delta_low, delta_high, step_input):
        ...
        def count_spikes(neuron):
            ...
            return sum([neuron(step_input[i]).item() for i in range(len(step_input))])
        
        low_spikes = count_spikes(delta_low)
        high_spikes = count_spikes(delta_high)
        
        assert low_spikes >= high_spikes, "Higher threshold produced more spikes"
