...
def test_run_leaky_zero_input():
    ...
    batch, time_steps, channels = 2, 3, 4
    x = torch.zeros(batch, time_steps, channels)

    spk_seq, mem_seq = run_leaky(layer, x)

    assert isinstance(spk_seq, torch.Tensor)
    assert isinstance(mem_seq, torch.Tensor)
    assert spk_seq.shape == (batch, time_steps, channels)
    assert mem_seq.shape == (batch, time_steps, channels)
...
def test_run_leaky_single_pulse_spikes_first_step():
    ...
    batch, time_steps, channels = 2, 4, 3
    x = torch.zeros(batch, time_steps, channels)
