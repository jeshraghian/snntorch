...
class StateLeakyCopyModel(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        h, _ = self.lif1(h)  # (T, B, H)
        h = h.reshape(-1, h.shape[-1])
        h = self.mid(h)  # (T*B, H)
        h = h.reshape(T, B, -1)
        h, _ = self.lif2(h)  # (T, B, H)
        h = h.reshape(-1, h.shape[-1])
...
def evaluate_stateleaky(
    ...
    model = StateLeakyCopyModel(seq_width=seq_width, hidden_dim=hidden_dim).to(
        DEVICE
    )
    model.eval()
    total_loss = 0.0
...
def main():
    ...
    ckpt_dir = os.path.join(
        os.path.dirname(__file__), "checkpoints_copytask_stateleaky"
    )
    ...
    def build_eval_set(num_batches: int = 50):
        ...
        __import__("random").seed(4321)
        __import__("numpy").random.seed(4321)
        data = list(
    ...
    for batch_idx, inp, target in tqdm(
        train_iter, total=total_steps, desc="StateLeaky train"
    ):
        step = batch_idx
        inp = inp.to(DEVICE)  # (L+1,B,seq_width+1)
        ...
        logits = model(x)  # (2L+1,B,W)
        y_write = logits[-L:]  # (L,B,W)
