...
def bench_stateleaky(
    num_steps, batch_size, channels, train=False, multi_beta=True
):
    beta = torch.full((channels,), 0.9).to(device) if multi_beta else 0.9
...
def run_all_configs_one_run(run_idx: int):
    ...
    for cfg in SWEEP_CONFIGS:
        ...
        for steps in tqdm(
            ...
            for bench_func, prefix in [
                (bench_leaky, "leaky"),
                (bench_stateleaky, "state"),
                (bench_gen2ssm, "gen2"),
            ]:
                for multi in [False, True]:
                    torch.cuda.synchronize()
                    torch.cuda.reset_peak_memory_stats(device)
                    baseline_mem, t = bench_func(
                        steps,
                        batch_size,
                        channels,
                        train=False,
                    ...
                    results_infer[
                        f"times_{prefix}_{'multi' if multi else 'single'}"
                    ].append(t)
                    results_infer[
                        f"mems_{prefix}_{'multi' if multi else 'single'}"
                    ].append(dmem)
            for bench_func, prefix in [
                (bench_leaky, "leaky"),
                (bench_stateleaky, "state"),
...
if __name__ == "__main__":
    ...
    for idx, res in enumerate(infer_sum):
        ...
        label_suffix = (
            f"B{infer_meta[idx]['batch_size']}-C{infer_meta[idx]['channels']}"
        )
        for prefix, fmt_single, fmt_multi in [
            ("leaky", "-", "-."),
            ("state", "--", ":"),
            ("gen2", "o-", "o--"),
        ]:
            ...
            ax_time_inf.errorbar(
                TIMESTEPS,
                res_mean[f"times_{prefix}_single"],
                yerr=res_std[f"times_{prefix}_single"],
                fmt=fmt_single,
                color=color,
                label=f"{prefix.capitalize()} single {label_suffix}",
                capsize=3,
            )
            ax_time_inf.errorbar(
                TIMESTEPS,
                res_mean[f"times_{prefix}_multi"],
                yerr=res_std[f"times_{prefix}_multi"],
                fmt=fmt_multi,
                color=color,
                label=f"{prefix.capitalize()} multi {label_suffix}",
                capsize=3,
            )
            ax_mem_inf.errorbar(
                TIMESTEPS,
                res_mean[f"mems_{prefix}_single"],
            ...
            ax_mem_inf.errorbar(
                TIMESTEPS,
                res_mean[f"mems_{prefix}_multi"],
    ...
    for idx, res in enumerate(train_sum):
        ...
        label_suffix = (
            f"B{train_meta[idx]['batch_size']}-C{train_meta[idx]['channels']}"
        )
        for prefix, fmt_single, fmt_multi in [
            ("leaky", "-", "-."),
            ("state", "--", ":"),
            ...
            ax_time_trn.errorbar(
                ...
                fmt=fmt_single,
                color=color,
                label=f"{prefix.capitalize()} single (train) {label_suffix}",
                capsize=3,
            )
            ax_time_trn.errorbar(
                TIMESTEPS,
                ...
                fmt=fmt_multi,
                color=color,
                label=f"{prefix.capitalize()} multi (train) {label_suffix}",
                capsize=3,
            )
            ax_mem_trn.errorbar(
                TIMESTEPS,
